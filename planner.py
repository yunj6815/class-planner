...
# 1. 페이지 설정 (사이드바 기본으로 열려있도록 설정)
st.set_page_config(page_title="스마트 진도표", layout="wide", initial_sidebar_state="expanded")
...
<style>
    ...
    div.element-container:has(span.edit-anchor) + div.element-container button {
        width: 100% !important; height: auto !important; min-height: 90px !important; display: block !important;
        padding: 10px !important; border-radius: 4px !important; border: 1px solid #ddd !important;
        box-shadow: 1px 1px 3px rgba(0,0,0,0.05) !important; color: #333 !important; text-align: left !important;
        margin-bottom: 4px !important; transition: transform 0.1s ease-in-out !important;
    }
    ...
    div.element-container:has(span.disabled-slot) + div.element-container button { 
        background-color: #f8f9fa !important; border: 1px solid #e9ecef !important; border-top: none !important;
        color: #adb5bd !important; box-shadow: none !important;
    }
...
with st.sidebar:
    ...
    # 1. 저장 및 복구
    if st.button("💾 현재 상태 전체 저장", type="primary", use_container_width=True):
        save_settings()
    ...
    with st.expander("📥 진도 계획 엑셀 백업 / 업로드", expanded=False):
        ...
        with pd.ExcelWriter(towrite, engine='xlsxwriter') as writer:
            for g in ["1학년", "2학년", "3학년"]:
                st.session_state.lesson_plans_dict[g].to_excel(writer, index=False, sheet_name=g)
        towrite.seek(0)

        st.download_button(
            label="🟢 전체 진도 계획 다운로드",
            data=towrite,
            file_name=f"진도계획_전체백업_{datetime.now().strftime('%Y%m%d')}.xlsx",
        ...
        st.write("**📂 백업 파일 불러오기**")
        uploaded_file = st.file_uploader("업로드하면 기존 진도표에 덮어씁니다.", type=['xlsx'])
        if uploaded_file is not None:
            if st.button("적용하기 (기존 데이터 덮어쓰기)", type="primary", use_container_width=True):
                try:
    ...
    with st.expander("📚 학년별 진도 내용 수정", expanded=False):
    ...
    with st.expander("🚨 전체 일정 변경 (행사/결강)", expanded=False):
